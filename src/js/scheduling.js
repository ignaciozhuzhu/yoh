...
var hosAddWork = function(doctorid, workdate, type, status, isClose, element) {
	...
	$.ajax({
		url: serviceurl + "workDay/hosAddWork",
		type: "post",
		dataType: "json",
			...
			if (dt.status == "success") {
				if (dt.data > 0) {
					if (window.confirm('是否要停诊已经有挂号的排班？')) {
						hosAddWork(doctorid, workdate, type, "1", "Y");
					} else {
						return;
					}
				} else {
					...
					function arrange(dayItem, noon, that) {
						...
						if (thisDate.getTime() > thatDate.getTime()) {
							...
							if (Vld.equals(that.attr("class"), "pointer")) {
								//可以排班
								hosAddWork(docID, date, noon, "0", "N", that);
								return;
